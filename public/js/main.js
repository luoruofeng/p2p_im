...
var pcConfig = {
  'iceServers': [{// 指定 ICE 服务器信令
    ...
    'username': "luoruofeng"//turn 服务器用户名
  }]
};
...
//客户端收到 joined 消息后，就要创建 RTCPeerConnection 对象了，也就是要建立一条与远端通话的音视频数据传输通道。
    ...
    socket.on('message', (roomid, data) => {
        ...
        }else if (data.hasOwnProperty('type') && data.type === 'candidate'){//通过信令服务器也可以将各端的 Candidate 交换完成了。
...
//首先要发送 join 消息，也就是要先进入房间。此时，如果服务器判定用户是合法的，则会给客户端回 joined 消息。
...
function createPeerConnection(){
    ...
    if(!pc){
        ...
        //当数据流过来的时候会触发 RTCPeerConnection 对象的 ontrack 事件，只要我们侦听该事件，并在回调函数中将收到的 track 与<video>标签绑定到一起就好了
